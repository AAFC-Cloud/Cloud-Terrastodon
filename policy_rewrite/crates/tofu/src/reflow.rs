...
pub async fn reflow_workspace(
    ...
    info!("Updating references from hardcoded IDs to resource attribute references");
    ...
    info!("Adding created blocks to body");
    for block in data_blocks.into_blocks() {
        body.push(block);
    }

    info!("Updating string literals to reference data blocks instead");
