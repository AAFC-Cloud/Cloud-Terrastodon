...
impl Slug for SubnetName {
    fn try_new(name: impl Into<CompactString>) -> eyre::Result<Self> {
        let inner = name.into();
        validate_subnet_name(&inner)?;
        Ok(Self { inner })
    }

    fn validate_slug(&self) -> eyre::Result<()> {
        validate_subnet_name(&self.inner)?;
        Ok(())
    }
}

fn validate_subnet_name(value: &str) -> eyre::Result<()> {
    validate_subnet_name_inner(value)
        .wrap_err_with(|| format!("Invalid subnet name: {}", value))
...
fn validate_subnet_name_inner(value: &str) -> eyre::Result<()> {
    ...
    if !first_char.is_alphanumeric() {
        bail!("Subnet name must start with alphanumeric character");
    }
    ...
    if !(last_char.is_alphanumeric() || *last_char == '_') {
        bail!("Subnet name must end with alphanumeric character or underscore");
    }
    ...
    for char_code in &chars {
        if !(char_code.is_alphanumeric()
            ...
            || *char_code == '-')
        {
            bail!("Subnet name can only contain alphanumerics, underscores, periods, and hyphens");
        }
    }
