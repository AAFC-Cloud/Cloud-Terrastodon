...
crate::impl_facet_string_proxy!(AzureApplicationGatewayResourceId, value => value.expanded_form());
...
cloud_terrastodon_registry::register_arbitrary!(AzureApplicationGatewayResourceId);
...
mod test {
    ...
    pub fn round_trip() -> eyre::Result<()> {
        for i in 0..100 {
            ...
            assert_eq!(
                facet_json::from_str::<AzureApplicationGatewayResourceId>(&json)?,
                id
            );
        }
        Ok(())
