...
fn validate_network_interface_resource_name_inner(value: &CompactString) -> eyre::Result<()> {
    ...
    if !(1..=80).contains(&char_count) {
        bail!("Network interface resource name must be between 1 and 80 characters");
    }
...
impl FromStr for AzureNetworkInterfaceResourceName {
    type Err = eyre::Error;

    fn from_str(s: &str) -> Result<Self, Self::Err> {
        AzureNetworkInterfaceResourceName::try_new(s)
    }
}

impl TryFrom<&str> for AzureNetworkInterfaceResourceName {
    type Error = eyre::Error;

    fn try_from(value: &str) -> Result<Self, Self::Error> {
        AzureNetworkInterfaceResourceName::try_new(value)
    }
}
...
impl serde::Serialize for AzureNetworkInterfaceResourceName {
...
impl<'de> serde::Deserialize<'de> for AzureNetworkInterfaceResourceName {
...
impl Deref for AzureNetworkInterfaceResourceName {
...
impl TryFrom<CompactString> for AzureNetworkInterfaceResourceName {
    type Error = eyre::Error;
...
impl<'a> Arbitrary<'a> for AzureNetworkInterfaceResourceName {
    fn arbitrary(u: &mut Unstructured<'a>) -> arbitrary::Result<Self> {
        ...
        AzureNetworkInterfaceResourceName::try_new(CompactString::from(name))
...
mod test {
    ...
    pub fn validation() -> eyre::Result<()> {
        assert!(AzureNetworkInterfaceResourceName::try_new("nic01").is_ok());
        assert!(AzureNetworkInterfaceResourceName::try_new("a").is_ok());
        assert!(AzureNetworkInterfaceResourceName::try_new("a-b-c").is_ok());
        assert!(AzureNetworkInterfaceResourceName::try_new("abc.def").is_ok());
        assert!(AzureNetworkInterfaceResourceName::try_new("abc_def").is_ok());
        assert!(AzureNetworkInterfaceResourceName::try_new("a".repeat(80)).is_ok());

        assert!(AzureNetworkInterfaceResourceName::try_new("-").is_err());
        assert!(AzureNetworkInterfaceResourceName::try_new("hi+hi").is_err());
        assert!(AzureNetworkInterfaceResourceName::try_new("").is_err());
        assert!(AzureNetworkInterfaceResourceName::try_new(".abc").is_err());
        assert!(AzureNetworkInterfaceResourceName::try_new("_abc").is_err());
        assert!(AzureNetworkInterfaceResourceName::try_new("abc-").is_err());
        assert!(AzureNetworkInterfaceResourceName::try_new("abc.").is_err());
        assert!(AzureNetworkInterfaceResourceName::try_new("a".repeat(81)).is_err());
        Ok(())
    }
    ...
    pub fn fuzz() -> eyre::Result<()> {
        for _ in 0..100 {
            ...
            let name = AzureNetworkInterfaceResourceName::arbitrary(&mut un)?;
