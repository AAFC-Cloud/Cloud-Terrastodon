...
impl GiteaInstanceUrl {
    pub fn try_new(value: impl Into<CompactString>) -> eyre::Result<Self> {
        ...
        Ok(Self {
            inner: CompactString::from(normalized),
        })
    }
    ...
    pub fn api_url(&self, endpoint: &str) -> String {
        if endpoint.starts_with("https://") || endpoint.starts_with("http://") {
            ...
            format!("{}{}", self.inner, endpoint)
        } else if endpoint.starts_with('/') {
            format!("{}/api/v1{}", self.inner, endpoint)
        } else {
            format!("{}/api/v1/{}", self.inner, endpoint)
        }
    }
    ...
    pub fn storage_key(&self) -> String {
        let digest = Hash::to_hex(&blake3::hash(self.inner.as_bytes()));
        let readable = self
            .inner
            .chars()
            .map(|ch| {
...
impl Serialize for GiteaInstanceUrl {
...
impl<'de> Deserialize<'de> for GiteaInstanceUrl {
