mod accounts;
mod app_service_list_request;
mod application_gateway_backend_health_request;
mod application_gateway_list_request;
mod application_registration_get_request;
mod application_registration_list_request;
mod application_registration_search_request;
mod application_role_list_request;
mod auth;
mod batch_api;
mod cognitive_services_account_deployment_list_request;
mod cognitive_services_account_list_request;
mod compute_publisher_vm_image_offer_sku_versions;
mod compute_publisher_vm_image_offer_skus;
mod compute_publisher_vm_image_offers;
mod compute_publishers;
mod compute_sku_prices;
mod compute_skus;
mod conditional_access_named_locations;
mod conditional_access_policies;
mod container_instance_list_request;
mod container_registry;
mod cost_management;
mod create_role_assignment;
mod eligible_child_resources;
mod entra_directory_objects_by_ids_request;
mod entra_group_get_request;
mod entra_group_list_request;
mod entra_group_member_add;
mod entra_group_member_remove;
mod entra_group_members;
mod entra_group_members_batch;
mod entra_group_owners;
mod entra_groups_for_member;
mod entra_user_get_request;
mod entra_user_list_request;
mod entra_user_pick_request;
mod entra_user_search_request;
mod evaluate_policy_assignment_compliance;
mod governance_role_assignments;
mod import;
mod key_vault_secrets;
mod key_vaults;
mod locations;
mod management_groups;
mod metrics;
mod microsoft_graph;
mod microsoft_graph_batch;
mod network_interface_list_request;
mod oauth2_permission_grant_create_request;
mod oauth2_permission_grant_list_request;
mod oauth2_permission_grant_update_request;
mod oauth2_permission_grants;
mod oauth2_permission_scopes;
mod percent_encode;
mod pick_oauth2_permission_grants;
mod pim_azurerm_activate;
mod pim_entra_activate;
mod pim_entra_role_assignments;
mod pim_entra_role_definitions;
mod pim_entra_role_settings;
mod policy_assignments;
mod policy_definitions;
mod policy_set_definitions;
mod principal_get_request;
mod principals;
mod private_endpoint_list_request;
mod public_ip_list_request;
mod remediate_policy_assignment;
mod remove_oauth2_permission_grant;
mod resource_graph;
mod resource_group_choices;
mod resource_group_list_request;
mod resources;
mod role_assignment_choices;
mod role_assignments;
mod role_definitions;
mod role_definitions_and_assignments;
mod role_eligibility_schedules;
mod role_management_policy_assignments;
mod role_operations;
mod route_table;
mod security_group_choices;
mod security_groups;
mod service_groups;
mod service_principal_get_request;
mod service_principals;
mod storage_account_blob_container_names;
mod storage_account_name_availability;
mod storage_accounts;
mod subscriptions;
mod tags;
mod tenant_default;
mod tenant_details;
mod tenant_licenses;
mod tracked_tenants;
mod unified_role_assignments;
mod unified_role_definition;
mod unified_role_definitions;
mod unified_role_definitions_and_assignments;
mod virtual_machine_prices;
mod virtual_machine_sizes;
mod virtual_machine_skus;
mod virtual_machines;
mod virtual_network;
pub use crate::accounts::*;
pub use crate::app_service_list_request::*;
pub use crate::application_gateway_backend_health_request::*;
pub use crate::application_gateway_list_request::*;
pub use crate::application_registration_get_request::*;
pub use crate::application_registration_list_request::*;
pub use crate::application_registration_search_request::*;
pub use crate::application_role_list_request::*;
pub use crate::auth::*;
pub use crate::batch_api::*;
pub use crate::cognitive_services_account_deployment_list_request::*;
pub use crate::cognitive_services_account_list_request::*;
pub use crate::compute_publisher_vm_image_offer_sku_versions::*;
pub use crate::compute_publisher_vm_image_offer_skus::*;
pub use crate::compute_publisher_vm_image_offers::*;
pub use crate::compute_publishers::*;
pub use crate::compute_sku_prices::*;
pub use crate::compute_skus::*;
pub use crate::conditional_access_named_locations::*;
pub use crate::conditional_access_policies::*;
pub use crate::container_instance_list_request::*;
pub use crate::container_registry::*;
pub use crate::cost_management::*;
pub use crate::create_role_assignment::*;
pub use crate::eligible_child_resources::*;
pub use crate::entra_directory_objects_by_ids_request::*;
pub use crate::entra_group_get_request::*;
pub use crate::entra_group_list_request::*;
pub use crate::entra_group_member_add::*;
pub use crate::entra_group_member_remove::*;
pub use crate::entra_group_members::*;
pub use crate::entra_group_members_batch::*;
pub use crate::entra_group_owners::*;
pub use crate::entra_groups_for_member::*;
pub use crate::entra_user_get_request::*;
pub use crate::entra_user_list_request::*;
pub use crate::entra_user_pick_request::*;
pub use crate::entra_user_search_request::*;
pub use crate::evaluate_policy_assignment_compliance::*;
pub use crate::governance_role_assignments::*;
pub use crate::import::*;
pub use crate::key_vault_secrets::*;
pub use crate::key_vaults::*;
pub use crate::locations::*;
pub use crate::management_groups::*;
pub use crate::metrics::*;
pub use crate::microsoft_graph::*;
pub use crate::microsoft_graph_batch::*;
pub use crate::network_interface_list_request::*;
pub use crate::oauth2_permission_grant_create_request::*;
pub use crate::oauth2_permission_grant_list_request::*;
pub use crate::oauth2_permission_grant_update_request::*;
pub use crate::oauth2_permission_grants::*;
pub use crate::oauth2_permission_scopes::*;
pub use crate::percent_encode::*;
pub use crate::pick_oauth2_permission_grants::*;
pub use crate::pim_azurerm_activate::*;
pub use crate::pim_entra_activate::*;
pub use crate::pim_entra_role_assignments::*;
pub use crate::pim_entra_role_definitions::*;
pub use crate::pim_entra_role_settings::*;
pub use crate::policy_assignments::*;
pub use crate::policy_definitions::*;
pub use crate::policy_set_definitions::*;
pub use crate::principal_get_request::*;
pub use crate::principals::*;
pub use crate::private_endpoint_list_request::*;
pub use crate::public_ip_list_request::*;
pub use crate::remediate_policy_assignment::*;
pub use crate::remove_oauth2_permission_grant::*;
pub use crate::resource_graph::*;
pub use crate::resource_group_choices::*;
pub use crate::resource_group_list_request::*;
pub use crate::resources::*;
pub use crate::role_assignment_choices::*;
pub use crate::role_assignments::*;
pub use crate::role_definitions::*;
pub use crate::role_definitions_and_assignments::*;
pub use crate::role_eligibility_schedules::*;
pub use crate::role_management_policy_assignments::*;
pub use crate::role_operations::*;
pub use crate::route_table::*;
pub use crate::security_group_choices::*;
pub use crate::security_groups::*;
pub use crate::service_groups::*;
pub use crate::service_principal_get_request::*;
pub use crate::service_principals::*;
pub use crate::storage_account_blob_container_names::*;
pub use crate::storage_account_name_availability::*;
pub use crate::storage_accounts::*;
pub use crate::subscriptions::*;
pub use crate::tags::*;
pub use crate::tenant_default::*;
pub use crate::tenant_details::*;
pub use crate::tenant_licenses::*;
pub use crate::tracked_tenants::*;
pub use crate::unified_role_assignments::*;
pub use crate::unified_role_definition::*;
pub use crate::unified_role_definitions::*;
pub use crate::unified_role_definitions_and_assignments::*;
pub use crate::virtual_machine_prices::*;
pub use crate::virtual_machine_sizes::*;
pub use crate::virtual_machine_skus::*;
pub use crate::virtual_machines::*;
pub use crate::virtual_network::*;
pub use cloud_terrastodon_azure_types::*;

cloud_terrastodon_registry::register_arbitrary!(());
