...
use cloud_terrastodon_azure_devops::prelude::AzureDevOpsTeam;
...
pub async fn browse_azure_devops_project_teams() -> Result<()> {
    ...
    let project =
        PickerTui::<AzureDevOpsProject>::new(projects.into_iter().map(|project| Choice {
            key: format!(
                "{} {:64} - {}",
                project.id,
                project.name,
    ...
    let teams = PickerTui::<AzureDevOpsTeam>::new(teams.into_iter().map(|team| Choice {
