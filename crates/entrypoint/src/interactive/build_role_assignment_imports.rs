...
pub async fn build_role_assignment_imports(auth_context: &AzureTenantAuthContext) -> Result<()> {
    info!("Fetching role assignments");
    let role_assignments = fetch_all_role_assignments(auth_context).await?;
